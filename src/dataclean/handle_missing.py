...
MISSING_STATS_OUTPUT_PATH = "data/processed/missing_stats.json"
...
def handle_missing_values(df, numeric_cols, text_cols, placeholder=TEXT_PLACEHOLDER):
    ...
    Modifies dataframe in place and returns it along with stats dictionary.
    """
    ...
    for col in tqdm(numeric_cols, desc=" Imputing numeric columns", leave=False): # Use leave=False for cleaner output in pipeline
        if col in df_handled.columns:
            ...
            if current_na_count > 0:
                ...
                df_handled[col] = df_handled[col].fillna(placeholder)
    ...
    stats = {
        "numeric_missing_imputed": total_missing_numeric,
        "text_missing_imputed": total_missing_text
    }
...
if __name__ == "__main__":
    ...
    reviews_df_handled, reviews_stats = handle_missing_values(reviews_df.copy(), reviews_numeric_cols, reviews_text_cols, TEXT_PLACEHOLDER)
    print(f"  Reviews data: Numeric missing values imputed: {reviews_stats['numeric_missing_imputed']}")
    print(f"  Reviews data: Text missing values imputed: {reviews_stats['text_missing_imputed']}")
    ...
    meta_df_handled, meta_stats = handle_missing_values(meta_df.copy(), meta_numeric_cols, meta_text_cols, TEXT_PLACEHOLDER)
    print(f"  Metadata: Numeric missing values imputed: {meta_stats['numeric_missing_imputed']}")
    print(f"  Metadata: Text missing values imputed: {meta_stats['text_missing_imputed']}")
    ...
    # Save missing stats to JSON file
    def convert_to_builtin_types(d):
        return {k: int(v) if isinstance(v, (np.integer, np.int64)) else v for k, v in d.items()}
    all_stats = {
        "reviews": convert_to_builtin_types(reviews_stats),
        "metadata": convert_to_builtin_types(meta_stats)
    }
    with open(MISSING_STATS_OUTPUT_PATH, "w") as f:
        json.dump(all_stats, f, indent=4)
    print(f"Missing value imputation statistics saved to {MISSING_STATS_OUTPUT_PATH}.")
